// Global Storage
lphillips_infernal_exalted_context = {};
...
function lphillips_infernal_exalted_dataPreLoad(opts){
...
function lphillips_infernal_exalted_dataPostLoad(opts){

	// Store global context for search by class name
	lphillips_infernal_exalted_context = document.getElementById(opts['containerId']);

	// Set additional options
	opts['imagePath'] = 'https://chainsawxiv.github.io/DST/sheets/lphillips_infernal_exalted/images/';
	opts['context'] = document.getElementById(opts['containerId']);
	opts['debugThreshold'] = -1;
	
	//Convert interface elements
	lphillips_infernal_exalted_convertLists(opts);
	lphillips_infernal_exalted_convertPips(opts);
	lphillips_infernal_exalted_convertChecks(opts);
	lphillips_infernal_exalted_convertAreas(opts);

}

// Called immediately before the sheet's data is saved
function lphillips_infernal_exalted_dataPreSave(opts) {

	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	lphillips_infernal_exalted_unconvertPips(opts);
	lphillips_infernal_exalted_unconvertChecks(opts);	
	lphillips_infernal_exalted_unconvertAreas(opts);
...
function lphillips_infernal_exalted_pips(oElement,opts){
...
function lphillips_infernal_exalted_convertPips(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/pips/)){
			pTemp = lphillips_infernal_exalted_pips(aSpans[i],opts);
			pTemp.showPips();
		}
	}
...
function lphillips_infernal_exalted_unconvertPips(opts) {
...
function lphillips_infernal_exalted_list(oElement,opts){
...
function lphillips_infernal_exalted_convertLists(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/list/)){
			...
			lTemp = lphillips_infernal_exalted_list(aDivs[i],opts);		
...
// Core Textarea Class
function lphillips_infernal_exalted_area(oElement,opts){
...
// Converts all properly classed divs in the context to lists
function lphillips_infernal_exalted_convertAreas(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/area/)){
			...
			// Convert each element to a full featured list object
			taTemp = lphillips_infernal_exalted_area(aSpans[i],opts);
			taTemp.activate();
...
function lphillips_infernal_exalted_unconvertAreas(opts){
...
// Applies a bunch of pips functionality to the specified element
function lphillips_infernal_exalted_check(oElement,opts){
...
// Converts all properly classed spans in the context to pips elements
function lphillips_infernal_exalted_convertChecks(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/check/)){
			pTemp = lphillips_infernal_exalted_check(aSpans[i],opts);
			pTemp.showCheck();
		}
	}
...
// Converts all the pips in the context back to thier basic values for saving
function lphillips_infernal_exalted_unconvertChecks(opts) {
...
function lphillips_infernal_exalted_getElementsByClassName(sClassName,sElementType){

	// Provide default element type
	if (!sElementType) sElementType = 'div';

	var aList = new Array();
	var aDivs = lphillips_infernal_exalted_context.getElementsByTagName(sElementType);
	for (var i = 0; i < aDivs.length; i++){
...
function lphillips_infernal_exalted_tabClick(oTab,sPageClass){
	
	// Abort if already on the selected tab
	var oPage = lphillips_infernal_exalted_getElementsByClassName('page_' + sPageClass)[0];
	if (oPage.style.display != 'none') return;
	
	// Set the length of the shorter page to match the longer page
	var oPages = lphillips_infernal_exalted_getElementsByClassName('main');
	var iHeight = 0;
	...
	var aTypes = lphillips_infernal_exalted_getElementsByClassName('dsf_type','span');
	if (sPageClass == 'sheet'){
	...
	if (sPageClass == 'sheet'){
		oTab.parentNode.style.backgroundImage = "url(https://chainsawxiv.github.io/DST/sheets/lphillips_infernal_exalted/images/brass-tab-left.png)";
		lphillips_infernal_exalted_getElementsByClassName('page_sheet')[0].style.display = 'block';
		lphillips_infernal_exalted_getElementsByClassName('page_bio')[0].style.display = 'none';
	}
	else{
		oTab.parentNode.style.backgroundImage = "url(https://chainsawxiv.github.io/DST/sheets/lphillips_infernal_exalted/images/brass-tab-right.png)";
		lphillips_infernal_exalted_getElementsByClassName('page_bio')[0].style.display = 'block';
		lphillips_infernal_exalted_getElementsByClassName('page_sheet')[0].style.display = 'none';	
	}
