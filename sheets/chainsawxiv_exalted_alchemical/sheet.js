// Global Storage
chainsawxiv_exalted_alchemical_context = {};
...
function chainsawxiv_exalted_alchemical_dataPreLoad(opts){
...
function chainsawxiv_exalted_alchemical_dataPostLoad(opts){
	...
	chainsawxiv_exalted_alchemical_convertLists(opts);
	chainsawxiv_exalted_alchemical_convertPips(opts);
	chainsawxiv_exalted_alchemical_convertChecks(opts);
	chainsawxiv_exalted_alchemical_convertAreas(opts);
	chainsawxiv_exalted_alchemical_convertTips(opts);

}

// Called immediately before the sheet's data is saved
function chainsawxiv_exalted_alchemical_dataPreSave(opts){

	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	chainsawxiv_exalted_alchemical_unconvertPips(opts);
	chainsawxiv_exalted_alchemical_unconvertChecks(opts);	
	chainsawxiv_exalted_alchemical_unconvertAreas(opts);
	
}

// Called after any jEditable field changes value
function chainsawxiv_exalted_alchemical_dataChange(opts){

	// Identify a data change within a tooltip area and unlock the tooltip
	oParent = chainsawxiv_exalted_alchemical_getElementsByClassName("dsf_" + opts['fieldName'],'span')[0].parentNode;
...
function chainsawxiv_exalted_alchemical_pips(oElement,opts){
...
function chainsawxiv_exalted_alchemical_convertPips(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/pips/)){
			pTemp = chainsawxiv_exalted_alchemical_pips(aSpans[i],opts);
			pTemp.showPips();
		}
	}
...
function chainsawxiv_exalted_alchemical_unconvertPips(opts) {
...
function chainsawxiv_exalted_alchemical_list(oElement,opts){
...
function chainsawxiv_exalted_alchemical_convertLists(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/list/)){
			...
			lTemp = chainsawxiv_exalted_alchemical_list(aDivs[i],opts);		
...
// Core Textarea Class
function chainsawxiv_exalted_alchemical_area(oElement,opts){
	...
	oElement.edit = function(){
		...
		var aAreas = chainsawxiv_exalted_alchemical_getElementsByClassName('area','span');
...
// Converts all properly classed divs in the context to lists
function chainsawxiv_exalted_alchemical_convertAreas(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/area/)){
			...
			// Convert each element to a full featured list object
			taTemp = chainsawxiv_exalted_alchemical_area(aSpans[i],opts);
			taTemp.activate();
...
function chainsawxiv_exalted_alchemical_unconvertAreas(opts){
...
// Applies a bunch of pips functionality to the specified element
function chainsawxiv_exalted_alchemical_check(oElement,opts){
...
// Converts all properly classed spans in the context to pips elements
function chainsawxiv_exalted_alchemical_convertChecks(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/check/)){
			pTemp = chainsawxiv_exalted_alchemical_check(aSpans[i],opts);
			pTemp.showCheck();
		}
	}
...
// Converts all the pips in the context back to thier basic values for saving
function chainsawxiv_exalted_alchemical_unconvertChecks(opts) {
...
function chainsawxiv_exalted_alchemical_tip(oElement,opts){
	...
	oElement.mouseOut = function(){
		...
		var hideRef = function(){chainsawxiv_exalted_alchemical_tip(oElement,opts).hideTip();};
...
function chainsawxiv_exalted_alchemical_convertTips(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/has_tooltip/)){
			pTemp = chainsawxiv_exalted_alchemical_tip(aDivs[i],opts);
			pTemp.activate();
		}
	}
...
function chainsawxiv_exalted_alchemical_getElementsByClassName(sClassName,sElementType){

	// Provide default element type
	if (!sElementType) sElementType = 'div';
	
	var aList = new Array();
	var aDivs = chainsawxiv_exalted_alchemical_context.getElementsByTagName(sElementType);
	for (var i = 0; i < aDivs.length; i++){
...
function chainsawxiv_exalted_alchemical_tabClick(oTab,sPageClass){
	
	// Abort if already on the selected tab
	var oPage = chainsawxiv_exalted_alchemical_getElementsByClassName('page_' + sPageClass)[0];
	if (oPage.style.display != 'none') return;
	
	// Set the length of the shorter page to match the longer page
	var oPages = chainsawxiv_exalted_alchemical_getElementsByClassName('main');
	var iHeight = 0;
	...
	var aTypes = chainsawxiv_exalted_alchemical_getElementsByClassName('dsf_type','span');
	if (sPageClass == 'sheet'){
