...
csx_opts = {
    'setupCallback': function(item){chainsawxiv_firefly_setup(item);},
...
// Master Startup
function chainsawxiv_firefly_dataPostLoad(data){
    ...
    includes.src = '../common/csx_exalted_common.js';
...
function chainsawxiv_firefly_setup(context){
    console.log("firefly_setup");

    // Provide default context
    if (context == undefined)
        context = csx_opts.defaultContext;

    csx_firefly_defaults(context);

    // Do setup for interfaces
    csx_firefly_dice(context);
    csx_check(context);
    csx_edit(context);
    csx_tip(context);
    //csx_list(context);
...
function chainsawxiv_firefly_dataPreSave(){

    console.log("dataPreSave");
    // Default the context if not set
    ...
    for (var i = 0; i < lists.length; i++)
        ...
        var dice = context.querySelectorAll('.dice_value');
    for (var i = 0; i < dice.length; i++){
        if (dice[i].parentNode.className.match(/proto/))
            continue;
        dice[i].unrender();
    }
...
var firefly_attributes = [ "mental", "physical", "social" ];
var firefly_skills= [
    "craft",
    "drive",
    "fight",
...
function csx_firefly_defaults(context) {
    for( var i = 0; i < firefly_skills.kength; i++ ) {
        var skill = firefly_skills[i];
        console.log("setting default for "+skill);
        var value = context.dynamic_sheet_attrs[skill];
        if( context.dynamic_sheet_attrs[skill] == null ) {
            context.dynamic_sheet_attrs[skill] = 4;
        }
    }
}

function csx_firefly_validate_die(value) {
    switch( value ) {
        case 4:
            case 6:
            case 8:
...
function csx_firefly_dice(context) {
    console.log("ff dice");
    var diceFields = context.querySelectorAll('.dice_value');
    console.log(diceFields);
    for (var i = 0; i < diceFields.length; i++) {
        var field = diceFields[i];
        field.small = /dice_small/.test(field.className);
        // Gets the current field value from text or image
        field.value = function(){
            // Return the cached value if it exists
            if(this.valueCache != null)
                return Number(this.valueCache.toString());
        ...
        if( !csx_firefly_validate_die(field.value()) ) {
            continue;
        }

        field.render = function() {
            var dice_class = "firefly_d" + field.value() + (field.small ? "-s" : "");
            $(field).removeClass (function (index, css) {
                return (css.match (/\bfirefly_d\S+[-s]?/g) || []).join(' ');
            }).addClass(dice_class);
        ...
        field.unrender = function() {
            $(field).removeClass (function (index, css) {
                return (css.match (/\bfirefly_d\S+/g) || []).join(' ');
            });
        }
