...
function chainsawxiv_vampire_dataPreLoad(opts){
...
function chainsawxiv_vampire_dataPostLoad(opts){
	...
	chainsawxiv_vampire_convertLists(opts);
	chainsawxiv_vampire_convertPips(opts);
	chainsawxiv_vampire_convertChecks(opts);
	chainsawxiv_vampire_convertAreas(opts);
	chainsawxiv_vampire_convertTips(opts);

}

// Called immediately before the sheet's data is saved
function chainsawxiv_vampire_dataPreSave(opts){

	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	chainsawxiv_vampire_unconvertPips(opts);
	chainsawxiv_vampire_unconvertChecks(opts);	
	chainsawxiv_vampire_unconvertAreas(opts);
	
}

// Called after any jEditable field changes value
function chainsawxiv_vampire_dataChange(opts){

	// Identify a data change within a tooltip area and unlock the tooltip
	oParent = chainsawxiv_vampire_getElementsByClassName("dsf_" + opts['fieldName'],'span')[0].parentNode;
...
function chainsawxiv_vampire_pips(oElement,opts){
...
function chainsawxiv_vampire_convertPips(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/pips/)){
			pTemp = chainsawxiv_vampire_pips(aSpans[i],opts);
			pTemp.showPips();
		}
	}
...
function chainsawxiv_vampire_unconvertPips(opts) {
...
function chainsawxiv_vampire_list(oElement,opts){
...
function chainsawxiv_vampire_convertLists(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/list/)){
			...
			lTemp = chainsawxiv_vampire_list(aDivs[i],opts);		
...
// Core Textarea Class
function chainsawxiv_vampire_area(oElement,opts){
	...
	oElement.edit = function(){
		...
		var aAreas = chainsawxiv_vampire_getElementsByClassName('area','span');
...
// Converts all properly classed divs in the context to lists
function chainsawxiv_vampire_convertAreas(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/area/)){
			...
			// Convert each element to a full featured list object
			taTemp = chainsawxiv_vampire_area(aSpans[i],opts);
			taTemp.activate();
...
function chainsawxiv_vampire_unconvertAreas(opts){
...
// Applies a bunch of pips functionality to the specified element
function chainsawxiv_vampire_check(oElement,opts){
...
// Converts all properly classed spans in the context to pips elements
function chainsawxiv_vampire_convertChecks(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/check/)){
			pTemp = chainsawxiv_vampire_check(aSpans[i],opts);
			pTemp.showCheck();
		}
	}
...
// Converts all the pips in the context back to thier basic values for saving
function chainsawxiv_vampire_unconvertChecks(opts) {
...
function chainsawxiv_vampire_tip(oElement,opts){
	...
	oElement.mouseOut = function(){
		...
		var hideRef = function(){chainsawxiv_vampire_tip(oElement,opts).hideTip();};
...
function chainsawxiv_vampire_convertTips(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/has_tooltip/)){
			pTemp = chainsawxiv_vampire_tip(aDivs[i],opts);
			pTemp.activate();
		}
	}
...
function chainsawxiv_vampire_getElementsByClassName(sClassName,sElementType){

	// Provide default element type
	if (!sElementType) sElementType = 'div';
	
	var aList = new Array();
	var aDivs = chainsawxiv_vampire_context.getElementsByTagName(sElementType);
	for (var i = 0; i < aDivs.length; i++){
...
function chainsawxiv_vampire_tabClick(oTab,sPageClass){
	
	// Abort if already on the selected tab
	var oPage = chainsawxiv_vampire_getElementsByClassName('page_' + sPageClass)[0];
	if (oPage.style.display != 'none') return;
	
	// Set the length of the shorter page to match the longer page
	var oPages = chainsawxiv_vampire_getElementsByClassName('main');
	var iHeight = 0;
	...
	var aTypes = chainsawxiv_vampire_getElementsByClassName('dsf_type','span');
	if (sPageClass == 'sheet'){
