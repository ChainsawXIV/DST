...
// Global Storage
lphillips_exalted_sidereal_context = {};
...
function lphillips_exalted_sidereal_dataPreLoad(opts){
...
function lphillips_exalted_sidereal_dataPostLoad(opts){
	...
	lphillips_exalted_sidereal_convertLists(opts);
	lphillips_exalted_sidereal_convertPips(opts);
	lphillips_exalted_sidereal_convertChecks(opts);
	lphillips_exalted_sidereal_convertAreas(opts);

}

// Called immediately before the sheet's data is saved
function lphillips_exalted_sidereal_dataPreSave(opts) {

	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	lphillips_exalted_sidereal_unconvertPips(opts);
	lphillips_exalted_sidereal_unconvertChecks(opts);	
	lphillips_exalted_sidereal_unconvertAreas(opts);
...
function lphillips_exalted_sidereal_pips(oElement,opts){
...
function lphillips_exalted_sidereal_convertPips(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/pips/)){
			pTemp = lphillips_exalted_sidereal_pips(aSpans[i],opts);
			pTemp.showPips();
		}
	}
...
function lphillips_exalted_sidereal_unconvertPips(opts) {
...
function lphillips_exalted_sidereal_list(oElement,opts){
...
function lphillips_exalted_sidereal_convertLists(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/list/)){
			...
			lTemp = lphillips_exalted_sidereal_list(aDivs[i],opts);		
...
// Core Textarea Class
function lphillips_exalted_sidereal_area(oElement,opts){
...
// Converts all properly classed divs in the context to lists
function lphillips_exalted_sidereal_convertAreas(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/area/)){
			...
			// Convert each element to a full featured list object
			taTemp = lphillips_exalted_sidereal_area(aSpans[i],opts);
			taTemp.activate();
...
function lphillips_exalted_sidereal_unconvertAreas(opts){
...
// Applies a bunch of pips functionality to the specified element
function lphillips_exalted_sidereal_check(oElement,opts){
...
// Converts all properly classed spans in the context to pips elements
function lphillips_exalted_sidereal_convertChecks(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/check/)){
			pTemp = lphillips_exalted_sidereal_check(aSpans[i],opts);
			pTemp.showCheck();
		}
	}
...
// Converts all the pips in the context back to thier basic values for saving
function lphillips_exalted_sidereal_unconvertChecks(opts) {
...
function lphillips_exalted_sidereal_getElementsByClassName(sClassName,sElementType){

	// Provide default element type
	if (!sElementType) sElementType = 'div';

	var aList = new Array();
	var aDivs = lphillips_exalted_sidereal_context.getElementsByTagName(sElementType);
	for (var i = 0; i < aDivs.length; i++){
...
function lphillips_exalted_sidereal_tabClick(oTab,sPageClass){
	
	// Abort if already on the selected tab
	var oPage = lphillips_exalted_sidereal_getElementsByClassName('page_' + sPageClass)[0];
	if (oPage.style.display != 'none') return;
	
	// Set the length of the shorter page to match the longer page
	var oPages = lphillips_exalted_sidereal_getElementsByClassName('main');
	var iHeight = 0;
	...
	var aTypes = lphillips_exalted_sidereal_getElementsByClassName('dsf_type','span');
	if (sPageClass == 'sheet'){
