...
// Global Storage
chainsawxiv_exalted_terrestrial_context = {};
...
function chainsawxiv_exalted_terrestrial_dataPreLoad(opts){
...
function chainsawxiv_exalted_terrestrial_dataPostLoad(opts){

	// Store global context for search by class name
	chainsawxiv_exalted_terrestrial_context = document.getElementById(opts['containerId']);

	// Set additional options
	opts['imagePath'] = 'https://chainsawxiv.github.io/DST/sheets/chainsawxiv_exalted_terrestrial/images/';
	opts['context'] = document.getElementById(opts['containerId']);
	opts['debugThreshold'] = -1;
	
	//Convert interface elements
	chainsawxiv_exalted_terrestrial_convertLists(opts);
	chainsawxiv_exalted_terrestrial_convertPips(opts);
	chainsawxiv_exalted_terrestrial_convertChecks(opts);
	chainsawxiv_exalted_terrestrial_convertAreas(opts);
	chainsawxiv_exalted_terrestrial_convertTips(opts);

}

// Called immediately before the sheet's data is saved
function chainsawxiv_exalted_terrestrial_dataPreSave(opts){

	// Set additional options
	opts['context'] = document.getElementById(opts['containerId']);

	chainsawxiv_exalted_terrestrial_unconvertPips(opts);
	chainsawxiv_exalted_terrestrial_unconvertChecks(opts);	
	chainsawxiv_exalted_terrestrial_unconvertAreas(opts);
	
}

// Called after any jEditable field changes value
function chainsawxiv_exalted_terrestrial_dataChange(opts){

	// Identify a data change within a tooltip area and unlock the tooltip
	oParent = chainsawxiv_exalted_terrestrial_getElementsByClassName("dsf_" + opts['fieldName'],'span')[0].parentNode;
	if (oParent.className.match('has_tooltip')){
...
function chainsawxiv_exalted_terrestrial_pips(oElement,opts){
	...
	oElement.showPips = function(){
		...
		if (this.getAttribute('optsImagePath')) var sPath = this.getAttribute('optsImagePath');
			...
			sPath = 'https://chainsawxiv.github.io/DST/sheets/chainsawxiv_exalted_terrestrial/images/';
		}
...
function chainsawxiv_exalted_terrestrial_convertPips(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/pips/)){
			pTemp = chainsawxiv_exalted_terrestrial_pips(aSpans[i],opts);
			pTemp.showPips();
		}
	}
...
function chainsawxiv_exalted_terrestrial_unconvertPips(opts) {
...
function chainsawxiv_exalted_terrestrial_list(oElement,opts){
...
function chainsawxiv_exalted_terrestrial_convertLists(opts){
	...
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/list/)){
			...
			lTemp = chainsawxiv_exalted_terrestrial_list(aDivs[i],opts);		
...
// Core Textarea Class
function chainsawxiv_exalted_terrestrial_area(oElement,opts){
	...
	oElement.edit = function(){
		...
		var aAreas = chainsawxiv_exalted_terrestrial_getElementsByClassName('area','span');
		...
		var iHeight = this.offsetHeight + parseInt(this.getAttribute('heightMod'));
		var sClasses = this.getAttribute('areaClasses');
		
		// Convert content into form with button
		this.innerHTML = '<textarea class="' + sClasses + '" style="width:' + iWidth + 'px;height:' + iHeight + 'px;">' + sText + '</textarea>';
		...
		// Set the user's focus to the textarea
...
// Converts all properly classed divs in the context to lists
function chainsawxiv_exalted_terrestrial_convertAreas(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/area/)){
			...
			// Convert each element to a full featured list object
			taTemp = chainsawxiv_exalted_terrestrial_area(aSpans[i],opts);
			taTemp.activate();
...
function chainsawxiv_exalted_terrestrial_unconvertAreas(opts){
...
// Applies a bunch of pips functionality to the specified element
function chainsawxiv_exalted_terrestrial_check(oElement,opts){
...
// Converts all properly classed spans in the context to pips elements
function chainsawxiv_exalted_terrestrial_convertChecks(opts){
	...
	for (var i = 0; i < aSpans.length; i++){
		if (aSpans[i].className.match(/check/)){
			pTemp = chainsawxiv_exalted_terrestrial_check(aSpans[i],opts);
			pTemp.showCheck();
		}
	}
...
// Converts all the pips in the context back to thier basic values for saving
function chainsawxiv_exalted_terrestrial_unconvertChecks(opts) {
...
function chainsawxiv_exalted_terrestrial_tip(oElement,opts){

	// Store options
	oElement.setAttribute('optsIsEditable',opts['isEditable']);
	oElement.setAttribute('optsDebugThreshold',opts['debugThreshold']);
	
	// Gets the tip display
	oElement.tip = function(){
		
		// Search the link's context for the tip element
	...
	// Assigns event functionality to a field
	oElement.activate = function(){
	
		// Register the hover events
		this.onmouseover = this.showTip;
		this.onmouseout = this.mouseOut;
		this.tip().onmouseout = this.mouseOut;
	
		// Don't actualy activate anything if we're not in edit mode
		if (this.getAttribute('optsIsEditable') != 'true') return;

		// Activate the element's edit event
	...
	// Shows the tool tip, event handler for mouseover
	oElement.showTip = function(){
	
		// If we're not in edit mode and the tip is empty, show nothing
		if (this.getAttribute('optsIsEditable') != 'true' && (this.tip().innerHTML == '' || this.tip().innerHTML == 'undefined')) return;
	
		// Clear the hide timer if we're still over the element
		if (this.timeout){
			clearTimeout(this.timeout);
			return;
		}
		
		// If we're editing the field, abort
		if (this.getAttribute("editLock") == "locked") return;
		
		// Capture the tool tip element
		var oTip = this.tip();
				
		// Position the tooltip under the main element
		oTip.style.top = this.offsetHeight + 'px';
	...
	// Hides the tool tip, event handler for mouseout
	oElement.hideTip = function(){
	
		// Abort the hide if we're editing
		if (this.getAttribute("editLock") == "locked") return;
	...
	// Handles the mouseout event for the tooltip
	oElement.mouseOut = function(){
		
		// Abort the event completely if we're editing
		if (this.getAttribute("editLock") == "locked") return;
	
		// Delays the hide by a 20th of a second
		// This allows mouseover to cancel hide for sub elements
		var hideRef = function(){chainsawxiv_exalted_terrestrial_tip(oElement,opts).hideTip();};
		this.timeout = setTimeout(hideRef,50);
		
	};
	
	// Click event handler for the tool tip interface
	oElement.mouseUp = function(e){
	
		// Cross browser event data getter
		var sClickedClass = '';
		if (!e) sClickedClass = window.event.srcElement.className;
		else sClickedClass = e.target.className;
		
		// Abort out if a non-jEditable thing was clicked
		if (!sClickedClass.match('dsf') || (sClickedClass.match('readonly') && !sClickedClass.match('area'))) return;
			
		// Hide the tooltip if that's not what we're editing
		if (!sClickedClass.match('is_tooltip')) this.hideTip();
	
		// If we're not already editing, lock the tooltip
		if (this.getAttribute("editLock") != "locked") this.setAttribute("editLock","locked");
...
// Converts all properly classed divs in the context to pips elements
function chainsawxiv_exalted_terrestrial_convertTips(opts){

	// Get all the divs in the context, whatever it may be
	if (opts['context']) var aDivs = opts['context'].getElementsByTagName('div');
	else var aDivs = document.getElementsByTagName('div');
	...
	var pTemp = {};
	for (var i = 0; i < aDivs.length; i++){
		if (aDivs[i].className.match(/has_tooltip/)){
			pTemp = chainsawxiv_exalted_terrestrial_tip(aDivs[i],opts);
			pTemp.activate();
		}
	}
...
function chainsawxiv_exalted_terrestrial_getElementsByClassName(sClassName,sElementType){

	// Provide default element type
	if (!sElementType) sElementType = 'div';
	
	var aList = new Array();
	var aDivs = chainsawxiv_exalted_terrestrial_context.getElementsByTagName(sElementType);
	for (var i = 0; i < aDivs.length; i++){
...
function chainsawxiv_exalted_terrestrial_tabClick(oTab,sPageClass){
	
	// Abort if already on the selected tab
	var oPage = chainsawxiv_exalted_terrestrial_getElementsByClassName('page_' + sPageClass)[0];
	if (oPage.style.display != 'none') return;
	
	// Set the length of the shorter page to match the longer page
	var oPages = chainsawxiv_exalted_terrestrial_getElementsByClassName('main');
	var iHeight = 0;
	...
	var aTypes = chainsawxiv_exalted_terrestrial_getElementsByClassName('dsf_type','span');
	if (sPageClass == 'sheet'){
	...
	if (sPageClass == 'sheet'){
		oTab.parentNode.style.backgroundImage = "url(https://chainsawxiv.github.io/DST/sheets/chainsawxiv_exalted_terrestrial/images/brass-tab-left.png)";
		chainsawxiv_exalted_terrestrial_getElementsByClassName('page_sheet')[0].style.display = 'block';
		chainsawxiv_exalted_terrestrial_getElementsByClassName('page_bio')[0].style.display = 'none';
	}
	else{
		oTab.parentNode.style.backgroundImage = "url(https://chainsawxiv.github.io/DST/sheets/chainsawxiv_exalted_terrestrial/images/brass-tab-right.png)";
		chainsawxiv_exalted_terrestrial_getElementsByClassName('page_bio')[0].style.display = 'block';
		chainsawxiv_exalted_terrestrial_getElementsByClassName('page_sheet')[0].style.display = 'none';	
	}
